...
def clean_stats(year: int, game_type: str, stats_category: str, drop_cols: list):
    ...
    with open(
        f"./data/raw_data/{stats_category}/{game_type}/{stats_category}_{year}.csv", "r", encoding="utf-8"
    ) as file:
        df = pd.read_csv(file)
...
def clean_all_def_awards(df: pd.DataFrame):
    ...
    all_def = df
    all_def["Player6"] = df["Player6"].astype(str)
    ...
    for index, row in all_def.iterrows():
        if "(T)" in row["Player5"]:
            ...
            names = val.split(" ")
            all_def.loc[index, "Player5"] = str(" ".join(names[:2]))
    ...
    for index, row in all_def_cleaned.iterrows():
        for i in range(1, 12):
            if i <= 5:
                all_def_cleaned.loc[index, f"all_def_{i}"] = all_def.query(f"Season == {row['Year']} and Tm == '1st'")[
                    f"Player{i}"
                ].iloc[0]
            elif 5 < i <= 10:
                all_def_cleaned.loc[index, f"all_def_{i}"] = all_def.query(f"Season == {row['Year']} and Tm == '2nd'")[
                    f"Player{i - 5}"
                ].iloc[0]
            else:
                if (
                    all_def.query(f"Season == {row['Year']} and Tm == '1st'")[f"Player{i-5}"]
                    .astype(str)
                    .iloc[0]
                    .lower()
                    == "nan"
                ):
                    all_def_cleaned.loc[index, f"all_def_{i}"] = all_def.query(
                        f"Season == {row['Year']} and Tm == '2nd'"
                    )[f"Player{6}"].iloc[0]
                else:
                    all_def_cleaned.loc[index, f"all_def_{i}"] = all_def.query(
                        f"Season == {row['Year']} and Tm == '1st'"
                    )[f"Player{6}"].iloc[0]
...
def clean_all_league_awards(df: pd.DataFrame):
    ...
    for index, row in all_league_cleaned.iterrows():
        for i in range(1, 16):
            if i <= 5:
                all_league_cleaned.loc[index, f"all_league_{i}"] = all_league.query(
                    f"Season == {row['Year']} and Tm == '1st'"
                )[f"Player{i}"].iloc[0]
            elif 5 < i <= 10:
                all_league_cleaned.loc[index, f"all_league_{i}"] = all_league.query(
                ...
                all_league_cleaned.loc[index, f"all_league_{i}"] = all_league.query(
                    f"Season == {row['Year']} and Tm == '3rd'"
                )[f"Player{i - 10}"].iloc[0]
...
def clean_personal_awards(pers_awards, dist_players: pd.DataFrame):
    ...
    player_names_cleaned = [x for x in player_names if x != "nan"]
...
def clean_salaries(year, dist_players: pd.DataFrame):
    ...
    distinct_players_not_accented["player"] = distinct_players_not_accented["player"].apply(
        lambda x: x.replace(".", "")
    )
    distinct_players_not_accented["player"] = distinct_players_not_accented["player"].apply(
        lambda x: x.replace("'", "")
    ...
    for index, row in players_salaries.iterrows():
        row_id = row["ID"]
        try:
            players_salaries.loc[index, "ID"] = distinct_players_not_accented.query(f'player == "{row_id}"')[
                "ID"
            ].iloc[0]
        except:
            if row_id[-3:] == " jr":
